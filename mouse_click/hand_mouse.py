...
def mouse_handler(l,x,y,ix,iy,w,h,new_img,cap_image,src):
    if x>280 : 
        if  20<l <25:
            ...
            src.append([x-(w-iy-300),y-100])
            time.sleep(0.3)
        if len(src)<3:
            for xx,yy in src:
                cv2.circle(new_img,center=(xx+(w-iy-300),yy+100),radius=5,color=(0,255,0),thickness=-1,lineType=cv2.LINE_AA)
...
def mouse_handler12(l,x,y,ix,iy,w,h,new_img,cap_image,src2):
    if x>280 : 
        if  20<l <25:
            cv2.putText(new_img,'clicked',(1100,600),cv2.FONT_HERSHEY_PLAIN,4,(0,255,0),3)
            src2.append([x-(w-iy-300),y-100])
            pygame.init()
            pygame.mixer.init()
            sound8 = pygame.mixer.Sound( path+"mouse_click.wav" )
            sound8.play()
            time.sleep(0.3)
...
def OCR(new_img,sc):
    ...
    while sc:
        try:
            ...
            txt_ko = txt_ko
            txt_p = translator.translate(txt, src='en', dest='ko')

            tts1 = gTTS(text=txt_p.text, lang='ko')
            tts1.save('trans.mp3')
            sc = False
            er = False
        except:
    ...
    if keys is not None:
        for j in range(len(keys)):
            for i in range(len(keys[j])):
                buttonList.append(Button([1000*i+70,80*j+50],keys[j][i]))
    if keys3 is not None:
        for j3 in range(len(keys3)):
            buttonList3.append(Button([1000,80*j3+50],keys3[j3]))
    if keys2 is not None:
        for j2 in range(len(keys2)):
            buttonList2.append(Button([70,80*j2+50],keys2[j2]))
